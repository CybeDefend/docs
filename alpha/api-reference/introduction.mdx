...
To obtain a JWT token, make a GET request to the `/auth/session` endpoint:

```bash
GET /auth/session
```

This endpoint requires you to be already authenticated (using session cookies). When successfully called, it returns your session information along with a JWT token that can be used for subsequent API calls:
