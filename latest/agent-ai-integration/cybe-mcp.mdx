...
| US | `https://mcp-us.cybedefend.com/mcp` | US region (Google Cloud). Controls aligned with SOC 2, Type II audit in progress. |
...
SAST, IaC, CI/CD and secret findings carry their rule code (`CD-<TYPE>-<NNNN>`) in `ruleCode`: it is the code an [inline `cybedefend-ignore` comment](/latest/code-scanning/scanning-options/inline-ignore-comments) takes to silence that rule.
