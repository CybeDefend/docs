...
SAST, IaC and CI/CD findings carry their rule code (`CD-<TYPE>-<NNNN>`) in `ruleCode`: it is the code an [inline `cybedefend-ignore` comment](/latest/code-scanning/scanning-options/inline-ignore-comments) takes to silence that rule.
