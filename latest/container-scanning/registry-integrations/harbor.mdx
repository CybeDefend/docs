...
|-------|-------------|---------|
| **Harbor URL** | Base URL of your Harbor instance — `https://` only, with no query string (`?`) or fragment (`#`) | `https://harbor.example.com` |
...
<Warning>
A stored credential whose **Harbor URL** is not `https://`, or carries a `?` or `#`, cannot start a scan. Open the credential in CybeDefend and correct the URL.
...
A bare host such as `harbor.example.com` is accepted and completed with `https://`.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="HTTPS Is Required" icon="lock">
    CybeDefend connects to Harbor over TLS only. A registry served over plain `http` cannot be used.
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="Host Does Not Answer as a Registry" icon="plug-circle-xmark">
    Saving a credential, browsing images and starting a scan all fail with **400** — *The host did not answer like a container registry … No credential was sent.* — when the **Harbor URL** does not reach a container registry. Nothing is sent to a host that fails this check, so your robot token stays unused.

    - Check the host for a typo, and enter the address of the Harbor instance itself.
  ...
  <Accordion title="Authentication Failed" icon="circle-exclamation">
    ...
    - Ensure the Harbor URL is correct, starts with `https://` and carries no `?` or `#`
  </Accordion>
