...
cybedefend-scan:
  stage: security
  image: ghcr.io/cybedefend/cybedefend-cli:v2.1.1
  variables:
...
<Tip>
To read the same policy data outside a scan — for a separate gate, a report or a
dashboard — use the [`cybedefend compliance` commands](/latest/code-scanning/local-code-scanning/cli-options#3-compliance):
`history`, `violations` and `stats`. Note that `compliance stats` returned zero
counters for every project before CLI **v2.1.1**; a gate built on it needs that
version or later to mean anything.
</Tip>
