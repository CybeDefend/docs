...
<Note>
**Scans that ran before 9 September 2026 never saw your `.cybeignore`.** The archive assembled for a scan keeps only recognised filenames, and `.cybeignore` carries no extension, so it was dropped before it reached the scanner — a repository using the recommended name logged `[IgnoreFilter] no .cybeignore file found` however the file was written. The deprecated `.cybedefend` name was not affected. Nothing to change on your side: re-run the scan and your patterns apply.
</Note>
