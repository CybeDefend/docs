...
`.cybeignore` filters **SAST** results only. Secret detection, SCA, IaC and CI/CD findings are produced by separate scanners and are **not** affected by this file — excluding a file here does not hide a hardcoded credential a later commit introduces into it.
