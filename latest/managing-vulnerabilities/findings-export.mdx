...
<Note>
`export_findings` is a read-only permission scoped to this feed. It does not require, and does not grant, any other permission on the platform. For a GRC or SIEM service account, grant `export_findings` alone.
</Note>

The export is limited to the projects the calling principal may read. The `project_id` filter can only **narrow** that set, never widen it.
