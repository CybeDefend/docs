...
2. **IAM Credentials**: AWS access keys with ECR permissions
3. **ECR Permissions**: `AmazonEC2ContainerRegistryReadOnly` policy (or `AmazonEC2ContainerRegistryFullAccess` for write)
4. **Region Configuration**: Specify the AWS region for your ECR repositories

## CybeDefend Configuration

When connecting your Amazon ECR in CybeDefend, you'll need to provide:

| Field | Description | Example |
|-------|-------------|---------|
| **AWS Account ID** | Your 12-digit AWS account ID | `123456789012` |
| **Région AWS** | AWS region where your ECR is located | `US East (N. Virginia) (us-east-1)` |
